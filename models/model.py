...
class TargetCoder(object):
    ...
    def encode(self, gt_K, gt_3Ds, gt_Rs, gt_Ts, anchors, bbox_trans=None):
        if self.regression_type == 'POINT':
            ...
            if bbox_trans is not None:
                # pts trans
                v_ones = torch.ones(ptx[0].size(0)).cuda()
                v_ones = v_ones.repeat(anchors.shape[0], 1)
                kpt2d = torch.stack([ptx, pty, v_ones]).transpose(0, 1)
                trans_kpt2d = torch.bmm(bbox_trans, kpt2d)
                ptx = trans_kpt2d[:, 0, :]
                pty = trans_kpt2d[:, 1, :]
    ...
    def decode(self, preds, anchors, bbox_trans=None):
        if self.regression_type == 'POINT':
            TO_REMOVE = 1  #
            anchors_w = anchors[:, 2] - anchors[:, 0] + TO_REMOVE
            anchors_h = anchors[:, 3] - anchors[:, 1] + TO_REMOVE
            ...
            if bbox_trans is not None:
                ...
                kpt2d = torch.stack([ptx, pty]).transpose(0, 1)
                R = bbox_trans[:, :, :2]
